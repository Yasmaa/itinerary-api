...
public class AddActivityToBucketListUseCase {
	...
	@Transactional
	public long addActivitytoBucketList(final String username, final long travelId, final long columnId,
		...
		Bucket b = bucketsRepository.findById(columnId).orElseThrow(EntityNotFoundException::new);
		Post ps = new Post();
		Cost cost = new Cost();
		...
		if (body.getStartLocation() != 0) {
			Location l1 = locationRepository.findById(body.getStartLocation())
		...
		Integer position = activityRepository.countBuckets(travelId, columnId);
		a.setBucketPosition(position);
