...
public class FindActivityCostsUseCase {
	...
	public Page<ActivityCost> findCosts(final long tripId, final long activityId, final int page, final int limit) {
		...
		return new PageImpl<ActivityCost>(costs.stream()
				...
				.map(this::convertToActivityCost).collect(Collectors.toList()), PageRequest.of(page, limit),
				totalElements);
	}

	private ActivityCost convertToActivityCost(Cost c) {
		modelMapper.getConfiguration().setMatchingStrategy(MatchingStrategies.LOOSE);
		ActivityCost ac = modelMapper.map(c, ActivityCost.class);
		return ac;
